...
class CreatePaymentOutput(ToolOutput):
    """创建支付订单输出"""
...
@register_tool
class CreatePaymentTool(AsyncTool[CreatePaymentInput, CreatePaymentOutput]):
    name = "create_payment"
    ...
    metadata = ToolMetadata(
        name="create_payment",
        description="创建支付订单工具",
        category="payment",
        tags=["payment", "wechat", "alipay"],
        rate_limit=100,
        timeout=30
    )

    async def async_execute(self, amount: float, payment_method: PaymentMethod, 
...
class ProcessPaymentOutput(ToolOutput):
    """处理支付输出"""
    order_id: str = Field(description="订单ID")
    transaction_id: str = Field(description="交易ID")
...
@register_tool
class ProcessPaymentTool(AsyncTool[ProcessPaymentInput, ProcessPaymentOutput]):
    name = "process_payment"
    description = "处理支付（模拟支付流程）"
    input_schema = ProcessPaymentInput
    output_schema = ProcessPaymentOutput
    metadata = ToolMetadata(
        name="process_payment",
        description="处理支付工具",
        category="payment",
    ...
    async def async_execute(self, order_id: str) -> ProcessPaymentOutput:
...
class QueryPaymentOutput(ToolOutput):
    """查询支付状态输出"""
    order_id: str = Field(description="订单ID")
    transaction_id: str = Field(description="交易ID")
    status: str = Field(description="支付状态")
...
@register_tool
class QueryPaymentTool(AsyncTool[QueryPaymentInput, QueryPaymentOutput]):
    name = "query_payment"
    description = "查询支付状态"
    input_schema = QueryPaymentInput
    output_schema = QueryPaymentOutput
    metadata = ToolMetadata(
        name="query_payment",
        description="查询支付状态工具",
        category="payment",
        tags=["payment", "query"],
        rate_limit=50,
        timeout=10
    ...
    async def async_execute(self, order_id: str) -> QueryPaymentOutput:
        order = await _payment_db.get_order(order_id)
        if not order:
...
@register_tool
class RefundPaymentTool(AsyncTool[RefundPaymentInput, RefundPaymentOutput]):
    name = "refund_payment"
    ...
    async def async_execute(self, order_id: str, refund_amount: Optional[float] = None) -> RefundPaymentOutput:
